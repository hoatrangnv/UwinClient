...
public class ViewElementGame : MonoBehaviour
{
    ...
    private void ShowEventJackpot()
    {
        ...
        if (isShowEvent)
        {
            if(DataResourceLobby.instance == null)
                StartCoroutine(FixIconEvent(maxEvent - 2));
            else
    ...
    IEnumerator FixIconEvent(int index)
    {
        while (DataResourceLobby.instance == null)
            yield return null;
        iconEvent.sprite = DataResourceLobby.instance.listSpiteEvent[index];
    }
