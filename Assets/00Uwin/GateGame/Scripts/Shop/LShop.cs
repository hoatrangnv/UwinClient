...
public class LShop : UILayer
{
    ...
    //My The Fake Shop
    public GameObject myShopStore, baseStore;
    ...
    {
        base.ShowLayer();
        int isShowMyFakeStore = PlayerPrefs.GetInt("ShowIAPStore", 1);
        if (isShowMyFakeStore == 1)
        {
            myShopStore.SetActive(true);
            baseStore.SetActive(false);
        }
        else
        {
            myShopStore.SetActive(false);
